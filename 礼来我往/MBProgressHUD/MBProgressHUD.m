...
- (void)showUsingAnimation:(BOOL)animated {
	self.alpha = 0.0f;
	if (animated && _animationType == MBProgressHUDAnimationZoom) {
...
- (void)done {
	isFinished = YES;
	self.alpha = 0.0f;
	if (_delegate && [_delegate respondsToSelector:@selector(hudWasHidden:)]) {
...
- (void)updateIndicators {
	...
	if (_mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
...
- (void)layoutSubviews {
	...
	// Position elements
	CGFloat yPos = roundf(((bounds.size.height - totalSize.height) / 2)) + _margin + _yOffset;
...
- (void)drawRect:(CGRect)rect {
	...
	CGRect boxRect = CGRectMake(roundf((allRect.size.width - _size.width) / 2) + self.xOffset,
								roundf((allRect.size.height - _size.height) / 2) + self.yOffset, _size.width, _size.height);
	float radius = 10.0f;
...
- (void)updateUIForKeypath:(NSString *)keyPath {
	if ([keyPath isEqualToString:@"mode"] || [keyPath isEqualToString:@"customView"]) {
		...
		if ([_indicator respondsToSelector:@selector(setProgress:)]) {
			[(id)_indicator setProgress:_progress];
		}
		return;
	}
	[self setNeedsLayout];
